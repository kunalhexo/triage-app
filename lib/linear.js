...
const DEFAULT_TEAM_KEY = process.env.LINEAR_TEAM_KEY || "TRI";
...
/* One call, up to 250 issues, everything the list needs — including enough
   ...
   `teamKey` is optional and lets the same deployment view a sandbox team —
   pass it from a `?team=` query param when testing, and it defaults to the
   real Triage team otherwise. This mirrors the TEAM: override the routines
   accept in their trigger text, so testing looks the same on both sides. */
export async function listIssues(teamKey = DEFAULT_TEAM_KEY) {
  const d = await gql(
    `query($key:String!){
      issues(
  ...
  return d.issues.nodes.map((n) => {
    const { score, rank } = extractRankAndScore(n.comments.nodes);
    return {
      id: n.id,
      key: n.identifier,
      title: n.title,
