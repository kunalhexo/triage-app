...
/* The app has never created a new issue before — every ticket so far has
   come from 1B. This exists for exactly one case: the magic box spawning a
   genuine follow-up ticket when a resolution implies separate future work
   (the Amir/Rockyard pattern — TRI-468 did this by hand once; this makes it
   the general mechanism). Deliberately narrow: title, description, and
   labels only. No priority, no score, no due date — those are 1B's
   judgment to make the next time this ticket gets triaged normally, not
   something invented here. */
export async function createIssue({ title, description = "", labelNames = [] }) {
  const { teamId, byName } = await getLabels();
  ...
  const d = await gql(
    ...
    { input: { teamId, title, description, labelIds } }
  );
  if (!d.issueCreate.success) throw new Error("Failed to create the follow-up issue.");
  return { id: d.issueCreate.issue.id, key: d.issueCreate.issue.identifier, url: d.issueCreate.issue.url };
}
