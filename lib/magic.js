...
function buildPrompt({ ticket, entities, entitiesError, userText }) {
  ...
  // Resolved in Pacific time, matching 1B's own convention (its Step 4 does
  // the same relative-date math against a Pacific reference point) — not
  // the server's own timezone, which on Vercel is UTC and would shift the
  // date near midnight boundaries. This was missing entirely before, and
  // it was a real, live bug: "remind me after 11th September" resolved to
  // 2025-09-11 — a year already past — because the classifier had no
  // actual reference point and had to guess.
...
TODAY'S DATE: ${todayPacific} (Pacific time — resolve every relative date
against this exact value, never against your own training data or any other
assumption about what year it is. "After 11th September", "next week", "in
two weeks" are all relative to this date specifically.)
