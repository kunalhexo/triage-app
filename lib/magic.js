...
function buildPrompt({ ticket, entities, userText }) {
...
- "quiet" — feedback, a lesson, or a correction with nothing to draft or
  send. If it implies you should check back later (a promise to have a
  conversation, a "waiting to hear back"), set impliesFollowUp and propose a
  sensible date.
...
- "resolution" — this ticket is done. If it clearly implies separate future
  ...
  continuing.
- "needs_research" — you would need to actually search Gmail, Slack, or the
  web to do this correctly, and guessing would be worse than admitting that.
...
export async function classify({ ticket, entities, userText }) {
  ...
  const res = await fetch("https://api.anthropic.com/v1/messages", {
    ...
    body: JSON.stringify({
      model: MODEL,
      max_tokens: 1000,
      messages: [{ role: "user", content: buildPrompt({ ticket, entities, userText }) }],
    }),
  });
